...
def load_distances_hdf5(filepath, load_full=False):
    """
    Load distances from HDF5 file
    
    Parameters:
    -----------
    filepath : str
    load_full : bool
        If True, load entire array into memory
        If False, return memory-mapped file handle (access on-demand)
    
    Returns:
    --------
    distances : array or h5py.Dataset
    majority_ids : array
    minority_ids : array
    """
    ...
    if load_full:
        ...
        # Return file handle (memory-mapped access)
        distances = f['distances']
        majority_ids = f['majority_enrolids'][:]
        minority_ids = f['minority_enrolids'][:]
        # Note: Keep f open, user must close it
    
    return distances, majority_ids, minority_ids


def load_distances_parquet(filepath, filters=None):
    """
    Load distances from Parquet file
    
    Parameters:
    -----------
    filepath : str
    filters : list of tuples, optional
        PyArrow filters, e.g., [('distance', '<', 2.0)]
    
    Returns:
    --------
    df : DataFrame with columns [majority_enrolid, minority_enrolid, distance]
    """
    table = pq.read_table(filepath, filters=filters)
    return table.to_pandas()


def load_distances_numpy_memmap(base_filepath, mode='r'):
    """
    Load distances from numpy memmap files
    
    Parameters:
    -----------
    base_filepath : str
    mode : str, 'r' (read-only), 'r+' (read-write), 'c' (copy-on-write)
    
    Returns:
    --------
    distances : memmap array (lazy-loaded)
    majority_ids : array
    minority_ids : array
    """
    distances = np.load(f"{base_filepath}_distances.npy", mmap_mode=mode)
    majority_ids = np.load(f"{base_filepath}_majority_ids.npy")
    minority_ids = np.load(f"{base_filepath}_minority_ids.npy")
...
def get_distances_for_minority_case(distances, minority_ids, case_enrolid, k=None):
    """
    Get all (or top-k nearest) majority controls for a given minority case
    
    Parameters:
    -----------
    distances : array (n_majority, n_minority)
    minority_ids : array of minority ENROLIDs
    case_enrolid : int, the minority case ENROLID
    k : int, optional, return only k-nearest
    
    Returns:
    --------
    indices : array of majority indices
    dists : array of distances
    """
    # Find column index for this case
    case_idx = np.where(minority_ids == case_enrolid)[0][0]
    
    # Get all distances to this case
    dists = distances[:, case_idx]
...
def get_distances_for_majority_control(distances, majority_ids, control_enrolid):
    """
    Get all distances from a majority control to all minority cases
    
    Parameters:
    -----------
    distances : array (n_majority, n_minority)
    majority_ids : array of majority ENROLIDs
    control_enrolid : int, the majority control ENROLID
    
    Returns:
    --------
    dists : array of distances to all minority cases
    """
    # Find row index for this control
    control_idx = np.where(majority_ids == control_enrolid)[0][0]
    return distances[control_idx, :]
